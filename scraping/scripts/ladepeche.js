...
for (const day of missing_days) {
  ...
  await delay(500)
  const url_to_fetch = `https://www.ladepeche.fr/articles/${format(day, 'yyyy/MM/dd', new Date())}/`
  ...
  $('ul').each((id, ref) => {
    ...
    if (is_relevant(title) === true) {
      const article = {
        title: title,
        snippet: null,
        url: `https://www.ladepeche.fr/articles${url_article}`,
        query: url_to_fetch,
        publisher: 'www.ladepeche.fr',
...
update_file['ladepeche'] = Date.now()
