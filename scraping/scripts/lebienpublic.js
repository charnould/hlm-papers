...
for (const day of missing_days) {
  ...
  const url_to_fetch = `https://www.bienpublic.com/archives/${getYear(day)}/${format(day, 'dd-MM', new Date())}`
  ...
  $('article.tertiary').each((id, ref) => {
    ...
    if (is_relevant(title) === true) {
      const article = {
        title: title,
        snippet: snippet,
        url: `https://www.bienpublic.com${url_article}`,
        query: url_to_fetch,
        publisher: 'www.bienpublic.com',
...
update_file['lebienpublic'] = Date.now()
