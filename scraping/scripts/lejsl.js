...
for (const day of missing_days) {
  ...
  const url_to_fetch = `https://www.lejsl.com/archives/${getYear(day)}/${format(day, 'dd-MM', new Date())}`
  ...
  $('article.tertiary').each((id, ref) => {
    ...
    if (is_relevant(title) === true) {
      const article = {
        title: title,
        snippet: snippet,
        url: `https://www.lejsl.com${url_article}`,
        query: url_to_fetch,
        publisher: 'www.lejsl.com',
