...
for (const day of missing_days) {
  ...
  const url_to_fetch = `https://www.midilibre.fr/articles/${format(day, 'yyyy/MM/dd', new Date())}/`
  ...
  $('ul').each((id, ref) => {
    ...
    if (is_relevant(title) === true) {
      const article = {
        title: title,
        snippet: null,
        url: `https://www.midilibre.fr/articles${url_article}`,
        query: url_to_fetch,
        publisher: 'www.midilibre.fr',
...
update_file['midilibre'] = Date.now()
