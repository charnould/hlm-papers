...
for (const day of missing_days) {
  ...
  const url_to_fetch = `https://www.vosgesmatin.fr/archives/${getYear(day)}/${format(day, 'dd-MM', new Date())}`
  ...
  $('article.tertiary').each((id, ref) => {
    ...
    if (is_relevant(title) === true) {
      const article = {
        title: title,
        snippet: snippet,
        url: `https://www.vosgesmatin.fr/${url_article}`,
        query: url_to_fetch,
        publisher: 'www.vosgesmatin.fr',
...
update_file['vosgesmatin'] = Date.now()
